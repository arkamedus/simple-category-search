...
class SimpleCategorySearch
{
    ...
    public function shortcode_handler($atts, $content = null)
    {
        ...
        $output .= '<input class="' . self::INPUT_CLASS . '" placeholder="Search..." />' . "\n";
    ...
    public function add_footer()
    {
        ...
        $site_url = site_url();
        $img = '<img src="%1$s" class="%2$s" style="float:left;width:16px;height:16px;" />';
        $loading_img = sprintf($img, admin_url('images/wpspin_light.gif'), 'loading');

        $output = '<script type="text/javascript">//<![CDATA[' . "\n";
        $output .= '(function($){$(document).ready(function() {';
        $output .= 'var i=$(".' . self::INPUT_CLASS . '");';
        $output .= 'var s=$("div.' . self::SELECT_CLASS . '");';
        $output .= 'var r=$("div.' . self::RESULT_CLASS . '");';

        $output .= 'var sc=function(){var a="parent"+this.id;var b=$(this).val();$("select."+a,s).fadeOut("fast",function(){$(this).remove()});
        $("ul",r).fadeOut("fast",function(){$(this).remove()});
        a+=" "+this.className;if(b!==""){var c={type:"json",parentcategory:b};$("button",r).css("visibility","hidden");
        s.append(\'' . $loading_img . '\');
        $.ajax({url:"' . $site_url . '",cache:false,type:"GET",dataType:"json",data:c,success:function(c){$("img.loading",s).remove();$("button",r).css("visibility","visible");
        if(c.count>0){var d=$(\'<select id="category-\'+b+\'" class="\'+a+\'"></select>\').hide();
        d.append($(\'<option value="" selected>All</option>\'));
        $.each(c.result,function(){d.append($(\'<option value="\'+this["term_id"]+\'">\'+this["name"]+"</option>"))});
        d.change(sc);s.append(d.fadeIn())}},error:function(){alert("error")}})}};';
        $output .= '$("select",s).change(sc);';
        $output .= '$("button",r).click(function(){var a="";jQuery("select",s).each(function(){if(jQuery(this).val()!=="")a=jQuery(this).val()});
        
        $("ul",r).fadeOut("fast",function(){$(this).remove()});if(a!==""){var b={type:"json",category:a,search:i.val()};
        $("button",r).css("visibility","hidden");r.append(\'' . $loading_img . '\');$.ajax({url:"' . $site_url . '",cache:false,type:"GET",dataType:"json",data:b,success:function(b){$("img.loading",r).remove();$("button",r).css("visibility","visible");if(b.count>0){var c=$(\'<ul id="category-\'+a+\'" style="clear:both;"></ul>\').hide();$.each(b.result,function(){c.append($(\'<li><a href="\'+this["permalink"]+\'" title="\'+this["title"]+\'">\'+this["title"]+"</a></li>"))});r.append(c.fadeIn())}},error:function(){alert("error")}})}});';
        /*
                $output .= <<<EOT
        var sc = function(){
